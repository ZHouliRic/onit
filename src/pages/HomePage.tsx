...
import { GOLDEN_CASE_PROJECT, GOLDEN_CASE_MESSAGES } from '@/lib/goldenCase'
import { createFreshProject } from '@/lib/projectFactory'

interface HomePageProps {
  onStartProject: (project: OnitProject, mode: 'custom' | 'demo') => void
}

const EXAMPLE_BRIEFS = [
  {
    label: '品牌社媒内容矩阵',
    brief: '帮我为一个新消费品牌搭建小红书+抖音的内容矩阵，包括选题规划、内容生产和发布排期。',
    tag: '内容创作',
  },
  {
    label: '产品发布全流程',
    brief: '我们要发布一款 SaaS 产品，需要准备产品文档、定价策略、落地页文案和 PR 稿。',
    tag: '产品发布',
  },
  {
    label: '用户调研报告',
    brief: '我需要对我们的核心用户做一次深度调研，包括问卷设计、访谈提纲、数据分析和结论报告。',
    tag: '用户研究',
  },
]
...
export default function HomePage({ onStartProject }: HomePageProps) {
  ...
  // ── Custom brief path ──────────────────────────────────────
  // Uses createFreshProject() — zero Golden Case dependency.
  // Enters UNDERSTANDING state immediately.
  const handleSubmit = async () => {
    if (!brief.trim()) return
    setIsLoading(true)
    await new Promise(resolve => setTimeout(resolve, 600))
    const project = createFreshProject(brief)
    setIsLoading(false)
    onStartProject(project, 'custom')
  }

  // ── Golden Case demo path ──────────────────────────────────
  // Explicitly loads the pre-built demo dataset.
  // Never used as a template for custom runs.
  const handleLoadGoldenCase = () => {
    const demoProject: OnitProject = {
      ...GOLDEN_CASE_PROJECT,
      messages: GOLDEN_CASE_MESSAGES,
    }
    onStartProject(demoProject, 'demo')
  }
    ...
    <div className="min-h-screen bg-[var(--color-background)] flex flex-col" style={{ background: 'var(--color-background)' }}>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-16" style={{ width: '100%' }}>
        <div style={{ width: '100%', maxWidth: '768px', margin: '0 auto' }}>
          ...
          {/* Example Briefs — custom path only, no Golden Case brief */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
