...
import { ArrowLeft, Zap, CheckCircle2, Clock, AlertCircle, Loader2, ChevronRight, FileText, Users } from 'lucide-react'
...
function ChatPanel({ messages, state }: { messages: ChatMessage[]; state: OnitProject['state'] }) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {(state === 'UNDERSTANDING' || state === 'PROPOSING') && (
...
function CanvasPanel({ project }: { project: OnitProject }) {
  ...
  return (
    <div className="flex flex-col h-full p-4 space-y-4 overflow-y-auto">
      ...
      {project.agents.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="flex items-center justify-between mb-3">
            ...
            {totalCount > 0 && <span className="text-xs text-[var(--color-muted-foreground)]">{completedCount}/{totalCount} 完成</span>}
          </div>
      ...
      {project.state === 'COMPLETE' && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="bg-[var(--color-card)] border border-[var(--color-success)] rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 size={16} className="text-[var(--color-success)]" />
            <p className="text-sm font-medium text-[var(--color-foreground)]">全部完成</p>
          </div>
          <p className="text-xs text-[var(--color-muted-foreground)]">{project.agents.length} 个 Agent 完成了所有任务，交付物已就绪。</p>
        </motion.div>
      )}
      {project.agents.length === 0 && (
...
export default function WorkspacePage({ project, onBack }: WorkspacePageProps) {
  const stateLabel: Record<OnitProject['state'], string> = {
    IDLE: '准备中', UNDERSTANDING: '理解需求中', PROPOSING: '制定方案中',
    CONFIRMING: '等待确认', EXECUTING: '执行中', BLOCKED: '等待决策', COMPLETE: '已完成',
  }
    ...
    <div className="h-screen flex flex-col bg-[var(--color-background)]">
      <header className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)] transition-colors">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-[var(--color-foreground)] max-w-[200px] truncate">
              {project.brief.slice(0, 30)}...
            </span>
          </div>
        </div>
        <div className={cn("text-xs font-medium flex items-center gap-1.5", stateColor[project.state])}>
          {(project.state === 'EXECUTING' || project.state === 'UNDERSTANDING' || project.state === 'PROPOSING') && <Loader2 size={12} className="animate-spin" />}
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 border-r border-[var(--color-border)] flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-hidden">
            <ChatPanel messages={project.messages} state={project.state} />
          </div>
        </div>
